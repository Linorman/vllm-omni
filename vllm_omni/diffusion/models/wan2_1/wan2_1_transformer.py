...
class WanFeedForward(nn.Module):
    """TP-enabled FeedForward network for Wan."""
...
class WanCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            # Wan cross-attn operates on short text-encoder sequences; per-block
...
class Wan21Transformer3DModel(nn.Module):
    """
    ...
    def __init__(
        ...
        if get_pipeline_parallel_world_size() == 0 and not is_pipeline_first_stage():
            raise RuntimeError(
                "`initialize_model_parallel()` must be called before constructing `Wan21Transformer3DModel`"
            )
        ...
        self.start_layer, self.end_layer, self.blocks = make_layers(
            ...
            lambda prefix: WanTransformerBlock(
                ...
                cross_attn_norm,
                quant_config=quant_config,
                prefix=f"{transformer_prefix}.{prefix}" if transformer_prefix else prefix,
            ),
            prefix="blocks",
        )
